...
async updateVoiceCommands(commands: { [key in string]: string }) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("update_voice_commands", { commands }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async updateAppPromptMap(map: { [key in string]: string }) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("update_app_prompt_map", { map }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async syncDictionaryNow() : Promise<Result<string[], string>> {
    try {
...
export type Snippet = { trigger: string; expansion: string }
